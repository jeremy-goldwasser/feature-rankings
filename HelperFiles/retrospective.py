...
def shapley_sampling(model, X, xloc, n_perms, mapping_dict=None, n_samples_per_perm=2, 
    ...
    if alphas is None:
        ...
        if isinstance(alphas, list) and len(alphas)>1:
            n_verified = [find_num_verified(shap_ests, shap_vars, alpha=alpha, abs=abs) for alpha in alphas]
        else:
            if isinstance(alphas, list): alphas = alphas[0]
            n_verified = find_num_verified(shap_ests, shap_vars, alpha=alphas, abs=abs)
