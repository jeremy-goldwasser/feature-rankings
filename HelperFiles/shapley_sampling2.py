...
def normal_test(feat1, feat2, alpha=0.05, multiplicative=False, scale_var=True):
    ...
    if Z_obs > Z_crit:
        ...
        if multiplicative:
            n_factor = (Z_crit/Z_obs)**2
            return "fail to reject", n_factor
        elif n1==n2:
            n_to_run = n1*(Z_crit/Z_obs)**2
            n_more_to_run = n_to_run - n1
        else:
            ...
            if n_more_to_run < 0:
                print(n_more_to_run)
                sys.exit(-1)
        return "fail to reject", n_more_to_run


def do_all_tests_pass(diffs_all_feats, K, alpha=0.05, 
                    order=None, multiplicative=False, scale_var=True):
    ...
    while first_test_to_fail < K:
        ...
        test_result = normal_test(feat1, feat2, alpha=alpha, 
                                multiplicative=multiplicative, scale_var=scale_var)
...
def shapley_sampling_adaptive(model, X, xloc, K, 
                            ...
                            buffer=1.1, multiplicative=False, 
                            max_n_perms=20000, scale_var=True):
    ...
    while not do_all_tests_pass(diffs_all_feats, K, alpha=alpha,
                            scale_var=scale_var, multiplicative=multiplicative):
        shap_ests = [np.mean(diffs_all_feats[j]) for j in range(d)]
        order = get_ordering(shap_ests)
        ...
        while test_result != "reject":
            ...
            n1, n2 = len(diffs_pair[0]), len(diffs_pair[1])
            if multiplicative:
                n_to_run = [n1*test_result[1], n2*test_result[1]]
            else:
                n_more_to_run = test_result[1]
                n_to_run = [n1+n_more_to_run, n2+n_more_to_run]
            n_to_run = [int(buffer*n) for n in n_to_run]
            # print(n_to_run)
